...
export const metadata: Metadata = {
    ...
    description: 'Découvrez mon portfolio, là où je montre tous les projets auxquels j\'ai participé. Développeur Web depuis 2013, j\'ai réalisé de nombreux projets, mais seuls les plus gros projets sont affichés.',
}
      ...
      <div className="mateo-grid bg-mateo-unique h-[100%]">
        <div className="flex flex-col gap-24 my-32 max-w-[1520px] px-6 mx-auto flex-1 w-full skewY-0 top-[-7rem] relative">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
            ...
            <div key={index} className={`relative group overflow-hidden bg-[#7e2e2a] hover:bg-[#33201f] rounded-[40px] w-full flex items-center justify-center z-0 col-span-1 aspect-square cursor-pointer ${project.columns === 1 ? '' : 'lg:col-span-2 lg:aspect-auto'}`} tabIndex={0} style={{ transform: 'none' }}>
                ...
                <img alt="logo" loading="lazy" width="1000" height="1000" decoding="async" data-nimg="1" className="h-28 w-auto max-w-[200px] object-contain" src={`${project.image}&v=1`} style={{ color: 'transparent' }} />
                <div className="absolute bottom-5 text-white text-sm text-center uppercase">{project.name}</div>
            </div>
            ))}
          </div>
        </div>
